...
namespace SchoolManagement.Services
{
    public class AdminService : IAdminService
    {
        ...
        public async Task<AdminStatisticsDto> GetOverallStatisticsAsync()
        {
            ...
            var totalSalaryPaid = await _context.Salaries.SumAsync(s => (decimal?)s.Amount) ?? 0;

            var hasSalaries = await _context.Salaries.AnyAsync();
            var averageSalary = hasSalaries
                ? await _context.Salaries.AverageAsync(s => s.Amount)
                : 0;

            var hasStudentCourses = await _context.StudentCourses.AnyAsync();
            var averageStudentMarks = hasStudentCourses
                ? await _context.StudentCourses.AverageAsync(sc => sc.StudentMark)
                : 0;
